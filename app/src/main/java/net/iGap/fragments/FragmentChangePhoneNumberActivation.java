...
import net.iGap.R;
import net.iGap.databinding.FragmentChangePhoneNumberActivationBinding;
...
import net.iGap.viewmodel.FragmentChangePhoneNumberActivationViewModel;
...
public class FragmentChangePhoneNumberActivation extends BaseFragment {

    private final static String TAG = FragmentChangePhoneNumberActivation.class.getName();

    private FragmentChangePhoneNumberActivationBinding binding;
    private FragmentChangePhoneNumberActivationViewModel viewModel;
    private SMSReceiver smsReceiver;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        viewModel = ViewModelProviders.of(this).get(FragmentChangePhoneNumberActivationViewModel.class);
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_change_phone_number_activation, container, false);
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.timerView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                binding.tvFragmentActivationPhoneNumberContainer.setTextColor(Theme.getColor(Theme.key_title_text));
                binding.timerPosition.setBackgroundColor(Theme.getColor(Theme.key_window_background));
        ...
        viewModel.closeFragment.observe(getViewLifecycleOwner(), isClose -> {
            if (isClose != null && isClose) {
                getEventManager().postEvent(EventManager.UPDATE_PHONE_NUMBER);
                finish();
            }
        });
    ...
    private void dialogWaitTime(WaitTimeModel data) {
        ...
        if (getActivity() != null && !getActivity().isFinishing()) {
            MaterialDialog dialogWait = new MaterialDialog.Builder(getActivity())
                    .titleColor(Theme.getColor(Theme.key_title_text))
                    .contentColor(Theme.getColor(Theme.key_default_text))
                    .title(data.getTitle()).customView(R.layout.dialog_remind_time, true).positiveText(R.string.B_ok).autoDismiss(false).canceledOnTouchOutside(false).cancelable(false).onPositive(new MaterialDialog.SingleButtonCallback() {
                @Override
