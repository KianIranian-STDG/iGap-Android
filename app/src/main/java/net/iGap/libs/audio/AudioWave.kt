...
class AudioWave : View {
    ...
    private fun inflateAttrs(attrs: AttributeSet?) {
        val resAttrs = context.theme.obtainStyledAttributes(
                attrs,
                R.styleable.AudioWave,
                0,
                0
        ) ?: return

        with(resAttrs) {
            chunkHeight = getDimensionPixelSize(R.styleable.AudioWave_chunkHeight, chunkHeight)
            chunkWidth = getDimensionPixelSize(R.styleable.AudioWave_chunkWidth, chunkWidth)
            chunkSpacing = getDimensionPixelSize(R.styleable.AudioWave_chunkSpacing,
                    chunkSpacing)
            minChunkHeight = getDimensionPixelSize(R.styleable.AudioWave_minChunkHeight,
                    minChunkHeight)
            chunkRadius = getDimensionPixelSize(R.styleable.AudioWave_chunkRadius, chunkRadius)
            isTouchable = getBoolean(R.styleable.AudioWave_touchable, isTouchable)
            waveColor = getColor(R.styleable.AudioWave_waveColor, waveColor)
            progress = getFloat(R.styleable.AudioWave_progress, progress)
            isExpansionAnimated = getBoolean(R.styleable.AudioWave_animateExpansion,
                    isExpansionAnimated)
            recycle()
        }
    }
