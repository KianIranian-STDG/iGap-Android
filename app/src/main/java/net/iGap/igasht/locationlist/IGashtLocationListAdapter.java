...
public class IGashtLocationListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
        if (viewHolder instanceof ViewHolderProvinceInfo) {
            ...
            Picasso.get().load(items.get(i-1).getmExtraDetail().getMainPicture()).placeholder(R.drawable.logo).error(R.drawable.ic_error_igap).fit().centerInside().into(((ViewHolderLocationItem) viewHolder).itemImageView);
