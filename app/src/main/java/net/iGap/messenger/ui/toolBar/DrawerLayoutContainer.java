...
import static net.iGap.BuildConfig.DEBUG;
import static net.iGap.module.AndroidUtils.density;
import static net.iGap.module.AndroidUtils.displayMetrics;
...
public class DrawerLayoutContainer extends FrameLayout {
    ...
    private ToolbarLayout parentToolbarLayout;
    ...
    public DrawerLayoutContainer(Context context) {
        ...
        minDrawerMargin = (int) (MIN_DRAWER_MARGIN * density + 0.5f);
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            setOnApplyWindowInsetsListener((v, insets) -> {
                ...
                if (statusBarHeight != insets.getSystemWindowInsetTop()) {
                    drawerLayoutContainer.requestLayout();
                }
                int newTopInset = insets.getSystemWindowInsetTop();
                if ((newTopInset != 0 || firstLayout) && statusBarHeight != newTopInset) {
    ...
    public void setDrawerPosition(float value) {
        ...
        if (!parentToolbarLayout.fragmentsStack.isEmpty()) {
            BaseFragment currentFragment = parentToolbarLayout.fragmentsStack.get(0);
        }
    ...
    public void setParentActionBarLayout(ToolbarLayout layout) {
        parentToolbarLayout = layout;
    }
    ...
    private void createBlurDrawable() {
        ...
        draw(canvas);
        stackBlurBitmap(bitmap, Math.max(7, Math.max(w, h) / 180));
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (drawCurrentPreviewFragmentAbove && parentToolbarLayout != null) {
            ...
            if (action == MotionEvent.ACTION_MOVE) {
                if (startY == 0) {
                    ...
                    parentToolbarLayout.movePreviewFragment(startY - ev.getY());
                }
            } else if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_POINTER_UP || action == MotionEvent.ACTION_CANCEL) {
                parentToolbarLayout.finishPreviewFragment();
            }
            return true;
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (drawerLayout != null) {
            ...
            if ((allowOpenDrawerBySwipe || drawerOpened) && allowOpenDrawer && parentToolbarLayout.fragmentsStack.size() == 1) {
                if (ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) && !startedTracking && !maybeStartTracking) {
                    ...
                    parentToolbarLayout.getHitRect(rect);
                    startedTrackingX = (int) ev.getX();
                    ...
                    if (maybeStartTracking && !startedTracking && (dx > 0 && dx / 3.0f > Math.abs(dy) && Math.abs(dx) >= getPixelsInCM(0.2f, true) || drawerOpened && dx < 0 && Math.abs(dx) >= Math.abs(dy) && Math.abs(dx) >= getPixelsInCM(0.4f, true))) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (Build.VERSION.SDK_INT < 21) {
            inLayout = true;
            if (heightSize == displaySize.y + statusBarHeight) {
                if (getLayoutParams() instanceof ViewGroup.MarginLayoutParams) {
                    setPadding(0, statusBarHeight, 0, 0);
                }
                heightSize = displaySize.y;
            } else {
                if (getLayoutParams() instanceof ViewGroup.MarginLayoutParams) {
                    setPadding(0, 0, 0, 0);
                }
            ...
            int newSize = heightSize - statusBarHeight;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (drawCurrentPreviewFragmentAbove && parentToolbarLayout != null) {
            if (previewBlurDrawable != null) {
                previewBlurDrawable.setAlpha((int) (parentToolbarLayout.getCurrentPreviewFragmentAlpha() * 255));
                previewBlurDrawable.draw(canvas);
            }
            parentToolbarLayout.drawCurrentPreviewFragment(canvas, Build.VERSION.SDK_INT >= 21 ? previewForegroundDrawable : null);
        }
    }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (drawingContent) {
            ...
            if (clipLeft != 0) {
                canvas.clipRect(clipLeft - LayoutCreator.dp(1), 0, clipRight, getHeight());
            }
        }
        ...
        if (scrimOpacity > 0 && drawingContent) {
            ...
            final float alpha = Math.max(0, Math.min(drawerPosition / LayoutCreator.dp(20), 1.0f));
            if (alpha != 0) {
    ...
    private static class PreviewForegroundDrawable extends Drawable {
        ...
        public PreviewForegroundDrawable() {
            ...
            topDrawable.setStroke(LayoutCreator.dp(1), Theme.getColor(Theme.key_toolbar_background));
            topDrawable.setCornerRadius(LayoutCreator.dp(6));
            bottomDrawable = new GradientDrawable();
            bottomDrawable.setStroke(1, Theme.getColor(Theme.key_light_gray));
            bottomDrawable.setCornerRadius(LayoutCreator.dp(6));
        }
        ...
        public void draw(@NonNull Canvas canvas) {
            ...
            canvas.save();
            canvas.clipRect(bounds.left, bounds.top, bounds.right, bounds.top + Toolbar.getCurrentActionBarHeight());
            topDrawable.draw(canvas);
            canvas.restore();

            canvas.save();
            canvas.clipRect(bounds.left, bounds.top + Toolbar.getCurrentActionBarHeight(), bounds.right, bounds.bottom);
