...
import net.iGap.messenger.ui.components.EmptyStubSpan;
import net.iGap.messenger.ui.components.StaticLayoutEx;
...
public class SimpleTextView extends View implements Drawable.Callback {
    ...
    private int drawablePadding = LayoutCreator.dp(4);
    ...
    public void setTextSize(int size) {
        int newSize = LayoutCreator.dp(size);
    ...
    public void setScrollNonFitText(boolean value) {
        ...
        if (scrollNonFitText) {
            fadePaint = new Paint();
            LinearGradient gradient = new LinearGradient(0, 0, LayoutCreator.dp(6), 0, new int[]{0xffffffff, 0}, new float[]{0f, 1f}, Shader.TileMode.CLAMP);
            fadePaint.setShader(gradient);
            fadePaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.DST_OUT));

            fadePaintBack = new Paint();
            gradient = new LinearGradient(0, 0, LayoutCreator.dp(6), 0, new int[]{0, 0xffffffff}, new float[]{0f, 1f}, Shader.TileMode.CLAMP);
                ...
                if (buildFullLayout) {
                    ...
                    if (!string.equals(text)) {
                        ...
                        if (fullLayout != null) {
                            ...
                            firstLineLayout = new StaticLayout(string, 0, string.length(), textPaint, scrollNonFitText ? LayoutCreator.dp(2000) : width + LayoutCreator.dp(8), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                            layout = new StaticLayout(substr, 0, substr.length(), textPaint, scrollNonFitText ? LayoutCreator.dp(2000) : width + LayoutCreator.dp(8), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                            if (layout.getLineLeft(0) != 0) {
                                part = "\u200F" + part;
                            }
                            partLayout = new StaticLayout(part, 0, part.length(), textPaint, scrollNonFitText ? LayoutCreator.dp(2000) : width + LayoutCreator.dp(8), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                            fullLayout = StaticLayoutEx.createStaticLayout(full, 0, full.length(), textPaint, width + LayoutCreator.dp(8) + fullLayoutAdditionalWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false, TextUtils.TruncateAt.END, width + fullLayoutAdditionalWidth, 3, false);
                        }
                    } else {
                        layout = new StaticLayout(string, 0, string.length(), textPaint, scrollNonFitText ? LayoutCreator.dp(2000) : width + LayoutCreator.dp(8), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                        fullLayout = null;
                        partLayout = null;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (lastWidth != displaySize.x) {
            lastWidth = displaySize.x;
            scrollingOffset = 0;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int nextScrollX = totalWidth + LayoutCreator.dp(DIST_BETWEEN_SCROLLING_TEXT);
        ...
        if (layout != null) {
            ...
            if (partLayout != null && fullAlpha < 1.0f) {
                ...
                canvas.save();
                if (layout.getLineLeft(0) != 0) {
                    canvas.translate(-layout.getLineWidth(0) + (partLayout.getText().length() == 1 ? LayoutCreator.dp(4) : 0), 0);
                } else {
                    canvas.translate(layout.getLineWidth(0) - (partLayout.getText().length() == 1 ? LayoutCreator.dp(4) : 0), 0);
                }
            ...
            if (fade) {
                if (scrollingOffset < LayoutCreator.dp(10)) {
                    fadePaint.setAlpha((int) (255 * (scrollingOffset / LayoutCreator.dp(10))));
                } else if (scrollingOffset > totalWidth + LayoutCreator.dp(DIST_BETWEEN_SCROLLING_TEXT) - LayoutCreator.dp(10)) {
                    float dist = scrollingOffset - (totalWidth + LayoutCreator.dp(DIST_BETWEEN_SCROLLING_TEXT) - LayoutCreator.dp(10));
                    fadePaint.setAlpha((int) (255 * (1.0f - dist / LayoutCreator.dp(10))));
                } else {
                    fadePaint.setAlpha(255);
                }
                canvas.drawRect(0, 0, LayoutCreator.dp(6), getMeasuredHeight(), fadePaint);
                canvas.save();
                canvas.translate(getMeasuredWidth() - LayoutCreator.dp(6), 0);
                canvas.drawRect(0, 0, LayoutCreator.dp(6), getMeasuredHeight(), fadePaintBack);
                canvas.restore();
            }
    ...
    private void updateScrollAnimation() {
        ...
        if (currentScrollDelay > 0) {
            ...
            int totalDistance = totalWidth + LayoutCreator.dp(DIST_BETWEEN_SCROLLING_TEXT);
            float pixelsPerSecond;
            if (scrollingOffset < LayoutCreator.dp(SCROLL_SLOWDOWN_PX)) {
                pixelsPerSecond = PIXELS_PER_SECOND_SLOW + (PIXELS_PER_SECOND - PIXELS_PER_SECOND_SLOW) * (scrollingOffset / LayoutCreator.dp(SCROLL_SLOWDOWN_PX));
            } else if (scrollingOffset >= totalDistance - LayoutCreator.dp(SCROLL_SLOWDOWN_PX)) {
                float dist = scrollingOffset - (totalDistance - LayoutCreator.dp(SCROLL_SLOWDOWN_PX));
                pixelsPerSecond = PIXELS_PER_SECOND - (PIXELS_PER_SECOND - PIXELS_PER_SECOND_SLOW) * (dist / LayoutCreator.dp(SCROLL_SLOWDOWN_PX));
            } else {
                pixelsPerSecond = PIXELS_PER_SECOND;
            }
            scrollingOffset += dt / 1000.0f * LayoutCreator.dp(pixelsPerSecond);
