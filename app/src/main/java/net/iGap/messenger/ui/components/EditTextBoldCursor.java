...
public class EditTextBoldCursor extends EditText {
    ...
    private void init() {
        ...
        errorPaint.setTextSize(LayoutCreator.dp(11));
        if (Build.VERSION.SDK_INT >= 26) {
            setImportantForAutofill(View.IMPORTANT_FOR_AUTOFILL_NO);
        }
        ...
        try {
            gradientDrawable = new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, new int[] {0xff8CC92F, 0xff8CC92F});
        ...
        cursorSize = LayoutCreator.dp(24);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (hintLayout != null) {
            setHintText(hint);
            lineY = (getMeasuredHeight() - hintLayout.getHeight()) / 2.0f + hintLayout.getHeight() + LayoutCreator.dp(6);
        }
    }
    ...
    public void setHintText(CharSequence text, boolean animated) {
        ...
        hintLayout = new StaticLayout(text, getPaint(), LayoutCreator.dp(1000), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if ((length() == 0 || transformHintToHeader) && hintLayout != null && (hintVisible || hintAlpha != 0)) {
            ...
            if (supportRtlHint && G.isAppRtl) {
                float offset = getMeasuredWidth() - hintWidth;
                canvas.translate(left + getScrollX() + offset, lineY - hintLayout.getHeight() - LayoutCreator.dp(6));
            } else {
                canvas.translate(left + getScrollX(), lineY - hintLayout.getHeight() - LayoutCreator.dp(6));
            }
            if (transformHintToHeader) {
                float scale = 1.0f - 0.3f * headerAnimationProgress;
                float translation = -LayoutCreator.dp(22) * headerAnimationProgress;
        ...
        try {
            ...
            if (allowDrawCursor && showCursor) {
                ...
                rect.right = bounds.left + LayoutCreator.dp(cursorWidth);
        ...
        if (lineColor != 0 && hintLayout != null) {
            ...
            if (!TextUtils.isEmpty(errorText)) {
                ...
                h = LayoutCreator.dp(2);
            } else if (isFocused()) {
                linePaint.setColor(activeLineColor);
                h = LayoutCreator.dp(2);
            } else {
                linePaint.setColor(lineColor);
                h = LayoutCreator.dp(1);
            }
    ...
    private void updateCursorPosition(int top, int bottom, float horizontal) {
        ...
        final int width = LayoutCreator.dp(cursorWidth);
