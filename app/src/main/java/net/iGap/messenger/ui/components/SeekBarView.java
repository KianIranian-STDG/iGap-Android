...
public class SeekBarView extends FrameLayout {
    ...
    public SeekBarView(Context context, boolean inPercents) {
        ...
        outerPaint1.setColor(Theme.getColor(Theme.key_button_background));

        selectorWidth = LayoutCreator.dp(32);
        thumbSize = LayoutCreator.dp(24);
        currentRadius = LayoutCreator.dp(6);

        if (Build.VERSION.SDK_INT >= 21) {
            hoverDrawable = Theme.createSelectorDrawable(ColorUtils.setAlphaComponent(Theme.getColor(Theme.key_button_background), 40), 1, LayoutCreator.dp(16));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        innerPaint1.setColor(Theme.getColor(Theme.key_window_background));
        canvas.drawRect(selectorWidth / 2, getMeasuredHeight() / 2 - LayoutCreator.dp(1), getMeasuredWidth() - selectorWidth / 2, getMeasuredHeight() / 2 + LayoutCreator.dp(1), innerPaint1);
        if (bufferedProgress > 0) {
            innerPaint1.setColor(Theme.getColor(Theme.key_light_theme_color));
            canvas.drawRect(selectorWidth / 2, getMeasuredHeight() / 2 - LayoutCreator.dp(1), selectorWidth / 2 + bufferedProgress * (getMeasuredWidth() - selectorWidth), getMeasuredHeight() / 2 + LayoutCreator.dp(1), innerPaint1);
        }
        if (twoSided) {
            canvas.drawRect(getMeasuredWidth() / 2 - LayoutCreator.dp(1), getMeasuredHeight() / 2 - LayoutCreator.dp(6), getMeasuredWidth() / 2 + LayoutCreator.dp(1), getMeasuredHeight() / 2 + LayoutCreator.dp(6), outerPaint1);
            if (thumbX > (getMeasuredWidth() - selectorWidth) / 2) {
                canvas.drawRect(getMeasuredWidth() / 2, getMeasuredHeight() / 2 - LayoutCreator.dp(1), selectorWidth / 2 + thumbX, getMeasuredHeight() / 2 + LayoutCreator.dp(1), outerPaint1);
            } else {
                canvas.drawRect(thumbX + selectorWidth / 2, getMeasuredHeight() / 2 - LayoutCreator.dp(1), getMeasuredWidth() / 2, getMeasuredHeight() / 2 + LayoutCreator.dp(1), outerPaint1);
            }
        } else {
            canvas.drawRect(selectorWidth / 2, getMeasuredHeight() / 2 - LayoutCreator.dp(1), selectorWidth / 2 + thumbX, getMeasuredHeight() / 2 + LayoutCreator.dp(1), outerPaint1);
        }
        if (hoverDrawable != null) {
            int dx = thumbX + selectorWidth / 2 - LayoutCreator.dp(16);
            int dy = y + thumbSize / 2 - LayoutCreator.dp(16);
            hoverDrawable.setBounds(dx, dy, dx + LayoutCreator.dp(32), dy + LayoutCreator.dp(32));
            hoverDrawable.draw(canvas);
        }
        boolean needInvalidate = false;
        int newRad = LayoutCreator.dp(pressed ? 8 : 6);
        ...
        if (currentRadius != newRad) {
            if (currentRadius < newRad) {
                currentRadius += LayoutCreator.dp(1) * (dt / 60.0f);
                if (currentRadius > newRad) {
