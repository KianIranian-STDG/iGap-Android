...
public class RadioButton extends View {
    ...
    private int size = LayoutCreator.dp(16);

    public RadioButton(Context context) {
        super(context);
        if (paint == null) {
            paint = new Paint(Paint.ANTI_ALIAS_FLAG);
            paint.setStrokeWidth(LayoutCreator.dp(2));
        ...
        try {
            bitmap = Bitmap.createBitmap(LayoutCreator.dp(size), LayoutCreator.dp(size), Bitmap.Config.ARGB_4444);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (bitmap != null) {
            ...
            if (progress <= 0.5f) {
                bitmapCanvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, (rad - LayoutCreator.dp(1)), checkedPaint);
                bitmapCanvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, (rad - LayoutCreator.dp(1)) * (1.0f - circleProgress), eraser);
            } else {
                bitmapCanvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, size / 4 + (rad - LayoutCreator.dp(1) - size / 4) * circleProgress, checkedPaint);
            }
