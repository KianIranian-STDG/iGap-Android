...
public abstract class AbstractMessage<Item extends AbstractMessage<?, ?>, VH extends RecyclerView.ViewHolder> extends AbstractItem<Item, VH> implements IChatItemAttachment<VH> {//IChatItemAvatar
    ...
    public static void processVideo(final TextView duration, final View holder1, final StructMessageInfo mMessage) {
        ...
        MediaController.onPercentCompress = (percent, path) -> {
    ...
    public void bindView(final VH holder, List<Object> payloads) {
        ...
        holder.itemView.setOnClickListener(view -> OnClickRow(mHolder, view));
                    ...
                    for (int i = 0; i < buttonList.size(); i++) {
                        ...
                        for (int j = 0; j < buttonList.get(i).length(); j++) {
                            try {
                                ...
                                childLayout = MakeButtons.addButtons(btnEntery, view -> {
                                    if (FragmentChat.isInSelectionMode) {
                                        holder.itemView.performLongClick();
                                        return;
                                    }
        ...
        if (!mMessage.isTimeOrLogMessage()) {
            ...
            if (realmRoom != null && realmRoom.getType() == ProtoGlobal.Room.Type.CHANNEL && ProtoGlobal.RoomMessageStatus.FAILED != ProtoGlobal.RoomMessageStatus.valueOf(mMessage.status)) {
                mHolder.getTicTv().setVisibility(View.GONE);
            } else {
                mHolder.getTicTv().setVisibility(View.VISIBLE);
                AppUtils.rightMessageStatus(mHolder.getTicTv(), ProtoGlobal.RoomMessageStatus.valueOf(mMessage.status), mMessage.forwardedFrom != null ? mMessage.forwardedFrom.getMessageType() : mMessage.messageType, mMessage.isSenderMe());
            }
        }
        /**
        ...
        if (HelperCalander.isPersianUnicode) {
            mHolder.getTimeTv().setText(HelperCalander.convertToUnicodeFarsiNumber(time));
        } else {
            mHolder.getTimeTv().setText(time);
        }
    ...
    private void voteSend(final ProtoGlobal.RoomMessageReaction reaction) {
        ...
        getRealmChat().executeTransaction(realm -> {
    ...
    protected void updateLayoutForReceive(VH holder) {
        ...
        ((FrameLayout.LayoutParams) mHolder.getItemContainer().getLayoutParams()).leftMargin = (int) holder.itemView.getContext().getResources().getDimension(R.dimen.dp4);
        ((FrameLayout.LayoutParams) mHolder.getItemContainer().getLayoutParams()).rightMargin = (int) holder.itemView.getContext().getResources().getDimension(R.dimen.dp28);
    ...
    protected void updateLayoutForSend(VH holder) {
        ...
        if (G.isDarkTheme) {
            ((View) (mHolder.getContentBloke()).getParent()).setBackgroundResource(R.drawable.rectangle_send_round_color_dark);
        } else {
            ((View) (mHolder.getContentBloke()).getParent()).setBackgroundResource(R.drawable.rectangle_send_round_color);
        }
        GradientDrawable circleDarkColor = (GradientDrawable) ((View) mHolder.getContentBloke().getParent()).getBackground();
        ...
        ((FrameLayout.LayoutParams) mHolder.getItemContainer().getLayoutParams()).rightMargin = (int) holder.itemView.getContext().getResources().getDimension(R.dimen.dp4);
    ...
    protected void forwardMessageIfNeeded(VH holder, Realm realm) {
        ...
        if (mMessage.forwardedFrom != null) {
            ...
            forwardView.setOnClickListener(v -> {
                if (FragmentChat.isInSelectionMode) {
                    holder.itemView.performLongClick();
                } else {
    ...
    private void setClickListener(SharedPreferences sharedPreferences, String key, final VH holder, final RealmAttachment attachment) {
        ...
        if (sharedPreferences.getInt(key, ((key.equals(SHP_SETTING.KEY_AD_DATA_GIF) || key.equals(SHP_SETTING.KEY_AD_WIFI_GIF)) ? 5 : -1)) != -1) {
            ...
            _Progress.withOnMessageProgress(progress -> forOnCLick(holder, attachment));
        }
    }
            ...
            if (hasProgress(holder)) {
                ...
                _Progress.withOnProgress(() -> {
                    ...
                    switch (messageType) {
                        ...
                        case IMAGE:
                        case IMAGE_TEXT:
                            thumbnailView.setOnClickListener(v -> forOnCLick(holder, attachment));
                            break;
                        case VOICE:
                            break;
                        case GIF:
            ...
            if (v.getId() == ButtonActionType.USERNAME_LINK) {
                ...
                try {
                    ...
                    realm.executeTransaction(realm1 -> {
                        RealmRoomMessage realmRoomMessage = RealmRoomMessage.makeAdditionalData(mMessage.roomId, identity, ((ArrayList<String>) v.getTag()).get(1).toString(), ((ArrayList<String>) v.getTag()).get(2).toString(), 3, realm1, ProtoGlobal.RoomMessageType.TEXT);
                    ...
                    new MaterialDialog.Builder(G.currentActivity).title(R.string.access_phone_number).positiveText(R.string.ok).negativeText(R.string.cancel).onPositive(new MaterialDialog.SingleButtonCallback() {
                        ...
                        public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                            ...
                            realm.executeTransaction(realm12 -> {
                                RealmUserInfo realmUserInfo = RealmUserInfo.getRealmUserInfo(realm12);
                                RealmRoomMessage realmRoomMessage = RealmRoomMessage.makeAdditionalData(mMessage.roomId, identity, realmUserInfo.getUserInfo().getPhoneNumber(), null, 0, realm12, ProtoGlobal.RoomMessageType.TEXT);
                                G.chatSendMessageUtil.build(type, mMessage.roomId, realmRoomMessage).sendMessage(identity + "");
                                messageClickListener.sendFromBot(realmRoomMessage);
                            });
                        }
