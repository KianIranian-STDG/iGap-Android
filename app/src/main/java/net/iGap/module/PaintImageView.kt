...
class PaintImageView @JvmOverloads constructor(context: Context,
    ...
    var paths: MutableList<Pair<Path, Paint>> = mutableListOf()
    val onPathChanged = MutableLiveData<Int>()

    init {
        if (attrs != null) {
            val typedArray = context.theme.obtainStyledAttributes(attrs,
                    R.styleable.PaintImageView, defStyleAttr, defStyleRes)
            try {
    ...
    private fun handleTouchStart(event: MotionEvent) {
        ...
        if (imageBounds.contains(event.x, event.y)) {
            ...
            onPathChanged.value = paths.size
            currentX = event.x
            currentY = event.y
        }
    }
