...
public class CheckBox extends View {
    ...
    private void init(){
        ...
        eraser2.setStrokeWidth(LayoutCreator.dp(28));
        backgroundPaint.setStrokeWidth(LayoutCreator.dp(2));
    ...
    public void setVisibility(int visibility) {
        ...
        if (visibility == VISIBLE && drawBitmap == null) {
            try {
                drawBitmap = Bitmap.createBitmap(LayoutCreator.dp(size), LayoutCreator.dp(size), Bitmap.Config.ARGB_4444);
                bitmapCanvas = new Canvas(drawBitmap);
                checkBitmap = Bitmap.createBitmap(LayoutCreator.dp(size), LayoutCreator.dp(size), Bitmap.Config.ARGB_4444);
    ...
    public void setSize(int size) {
        this.size = size;
        if (size == 40) {
            textPaint.setTextSize(LayoutCreator.dp(24));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawBackground || progress != 0) {
            eraser2.setStrokeWidth(LayoutCreator.dp(size + 6));
            ...
            if (roundProgressCheckState < progressBounceDiff) {
                rad -= LayoutCreator.dp(2) * roundProgressCheckState / progressBounceDiff;
            } else if (roundProgressCheckState < progressBounceDiff * 2) {
                rad -= LayoutCreator.dp(2) - LayoutCreator.dp(2) * (roundProgressCheckState - progressBounceDiff) / progressBounceDiff;
            }
            if (drawBackground) {
                paint.setColor(0x44000000);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, rad - LayoutCreator.dp(1), paint);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, rad - LayoutCreator.dp(1), backgroundPaint);
            }

            paint.setColor(color);

            if (hasBorder) {
                rad -= LayoutCreator.dp(2);
            }
            ...
            if (checkedText != null) {
                ...
                checkCanvas.drawText(checkedText, (getMeasuredWidth() - w) / 2, LayoutCreator.dp(size == 40 ? 28 : 21), textPaint);
            } else {
            ...
            checkCanvas.drawCircle(getMeasuredWidth() / 2 - LayoutCreator.dp(2.5f), getMeasuredHeight() / 2 + LayoutCreator.dp(4), ((getMeasuredWidth() + LayoutCreator.dp(6)) / 2) * (1 - checkProgress), eraser2);
